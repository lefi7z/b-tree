...
void test_count_simple_tree(void)
{
    int tree_data[] = { 3, 2, 4 };
    node_t* tree = build_tree(tree_data, ARRAY_SIZE(tree_data));

    int actual = count(tree);
    TEST_ASSERT_EQUAL_INT(3, actual);

    free_tree(tree);
}

void test_count_larger_tree(void)
{
    int tree_data[] = { 4, 2, 6, 1, 3, 5, 7 };
    node_t* tree = build_tree(tree_data, ARRAY_SIZE(tree_data));

    int actual = count(tree);
    TEST_ASSERT_EQUAL_INT(7, actual);
...
void test_find_node(void)
{
    ...
    node_t* candidate;
    candidate = find_first(tree, 4);
    TEST_ASSERT_NOT_NULL(candidate);
    TEST_ASSERT_EQUAL_INT(4, candidate->data);

    candidate = find_first(tree, 1);
    TEST_ASSERT_NOT_NULL(candidate);
    TEST_ASSERT_EQUAL_INT(1, candidate->data);

    candidate = find_first(tree, 5);
    TEST_ASSERT_NOT_NULL(candidate);
    TEST_ASSERT_EQUAL_INT(5, candidate->data);

    candidate = find_first(tree, 7);
    TEST_ASSERT_NOT_NULL(candidate);
    TEST_ASSERT_EQUAL_INT(7, candidate->data);

    candidate = find_first(tree, 13);
    TEST_ASSERT_NULL(candidate);
...
int main(void)
{
   ...
   RUN_TEST(test_count_simple_tree);
   RUN_TEST(test_count_larger_tree);
